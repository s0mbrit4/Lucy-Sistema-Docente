...
<body bgcolor=#E1DEDE>
<form action="./MostrarEstadisticas.php" method="POST">
<?php
$fecha= date ("j-n-Y");
...
?> 
 <table>
	...
	<tr><td>Liceo:</td><td><?php
                ...
                ?>
                <?php echo $result;?><select name="f1"><option value=0>Seleccione</option>
	...
	<tr><td>Grupo:</td><td><?php
                ...
                ?>
                <?php echo $result;?><select name="f2"><option value=0>Seleccione</option>
                    <?php foreach ($rows2 as $row2) {
			echo '<option value='.$row2['IdGrupo'].'>'.$row2['Grado'].'-'.$row2['Grupo'].'</option>';
                    } 
		mysql_close($conexion);?></select></td></tr>
	<tr><td>A&#241;o</td><td><input type="text" name="f3"></td></tr>
